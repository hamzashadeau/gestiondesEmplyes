package com.example.stock.ws.rest;

import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.DeleteMapping;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

import com.example.stock.bean.Banque;
import com.example.stock.service.facade.BanqueService;
@RestController
@RequestMapping("/gestionDesEmployee-Api/Banque/")
public class BanqueRest {
@Autowired
private BanqueService banqueService;

@GetMapping("findByid/id/{id}")
public Banque findByid(@PathVariable Long id) {
	return banqueService.findByid(id);
}
@GetMapping("findByLibelle/libelle/{libelle}")
public Banque findByLibelle(@PathVariable String libelle) {
	return banqueService.findByLibelle(libelle);
}
@GetMapping("findByRib/rib/{rib}")
public Banque findByRib(@PathVariable Double rib) {
	return banqueService.findByRib(rib);
}
@PostMapping("save")
public int save(@RequestBody Banque banque) {
	return banqueService.save(banque);
}

@GetMapping("findAll")
public List<Banque> findAll() {
	return banqueService.findAll();
}

@DeleteMapping("deleteById/id/{id}")
public int deleteById(@PathVariable Long id) {
	return banqueService.deleteById(id);
}
}
